...
1. **Check global repos** (core, skills):
   - For each directory in `~/.claude/repos/agentteamland/` that has a `.git/`:
...
2. **Check project team repos:**
   ...
   - For each installed team, check its `~/.claude/repos/agentteamland/{name}/` source
...
for team_dir in ~/.claude/repos/agentteamland/*/; do
